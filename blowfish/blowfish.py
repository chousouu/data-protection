...
print(len(decrypted_image_data))

with open('decrypted_image.png', 'wb') as decrypted_file:
    decrypted_file.write(decrypted_image_data)

with Image.open('decrypted_image.png') as img:
    width, height = img.size
    print(width, height)
    
print("Изображение расшифровано и сохранено как decrypted_image.png")

img_enc = Image.fromarray(np.frombuffer(encrypted_image_data[:277**2], dtype=np.uint8).reshape((277, 277)))
